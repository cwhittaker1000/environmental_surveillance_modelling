...
for (i in 1:length(beta_sens)) {
  ...
  for (j in 1:stochastic_sim) {
    ...
    beta_cuminf_output$first_flight_reads[counter] <- if(!is.na(beta_ttd_output$first_flight_reads[counter])) sum(output2$n_SE_Output[1:beta_ttd_output$first_flight_reads[counter]])/population_size else NA
    beta_cuminf_output$avg_flight_reads[counter] <- if(!is.na(beta_ttd_output$avg_flight_reads[counter])) sum(output2$n_SE_Output[1:beta_ttd_output$avg_flight_reads[counter]])/population_size else NA
    beta_cuminf_output$agg_flight_reads[counter] <- if(!is.na(beta_ttd_output$agg_flight_reads[counter])) sum(output2$n_SE_Output[1:beta_ttd_output$agg_flight_reads[counter]])/population_size else NA
    beta_cuminf_output$first_flight_inf[counter] <- if(!is.na(beta_ttd_output$first_flight_inf[counter])) sum(output2$n_SE_Output[1:beta_ttd_output$first_flight_inf[counter]])/population_size else NA
    beta_cuminf_output$avg_flight_inf[counter] <- if(!is.na(beta_ttd_output$avg_flight_inf[counter])) sum(output2$n_SE_Output[1:beta_ttd_output$avg_flight_inf[counter]])/population_size else NA
...
make_plot <- function(df, scaling_factor1, scaling_factor2, title) {
  
  temp1 <- ggplot(data = df) +
    geom_line(aes(x = R0, y = avg_ttd)) +
    geom_line(aes(x = R0, y = scaled_ttd)) +
  ...
  temp2 <- ggplot(data = df) +
    geom_line(aes(x = R0, y = avg_cuminf)) +
    geom_line(aes(x = R0, y = scaled_cuminf)) +
    geom_ribbon(aes(x = R0, y = avg_cuminf, ymin = lower_cuminf, ymax = upper_cuminf), alpha = 0.2) +
    lims(x = c(min(R0_df$R0), max(R0_df$R0))) + 
    scale_y_continuous(limits = c(0, NA), 
...
seq_tot_sens <- round(lseq(10^9, 10^12, 5))
seq_total_output <- data.frame(seq_total = rep(seq_tot_sens, each = stochastic_sim), stochastic_realisation = 1:stochastic_sim, num_reads = num_reads, 
                               first_flight_reads = NA, first_flight_reads_cuminf = NA,
                               avg_flight_reads = NA, avg_flight_reads_cuminf = NA, 
                               agg_flight_reads = NA, agg_flight_reads_cuminf = NA, 
                               first_flight_inf = NA, first_flight_inf_cuminf = NA, 
                               avg_flight_inf = NA, avg_flight_inf_cuminf = NA)
counter <- 1
for (i in 1:length(seq_tot_sens)) {
  ...
  for (j in 1:stochastic_sim) {
    ...
    mod <- stoch_seir_dust$new(# Epidemiological Parameters
      ...
      # Sequencing Parameters
      shedding_freq = 1, virus_shed = virus_shed, non_virus_shed = virus_shed * virus_ratio, met_bias = met_bias, 
    ...
    # Calculating Time to Detection
    seq_total_output$first_flight_reads[counter] <- min(time_to_detection_fun(output2, num_reads, "reads", "individual"))
    seq_total_output$avg_flight_reads[counter] <- mean(time_to_detection_fun(output2, num_reads, "reads", "individual")) # check whether this is correct or need to find when average line passes 5 for first time
    seq_total_output$agg_flight_reads[counter] <- time_to_detection_fun(output2, num_reads, "reads", "aggregated")
    seq_total_output$first_flight_inf[counter] <- min(time_to_detection_fun(output2, num_reads, "infections", "individual"))
    seq_total_output$avg_flight_inf[counter] <- mean(time_to_detection_fun(output2, num_reads, "infections", "individual"))
    
    # Calculating Cumulative Incidence at Time to Detection
    seq_total_output$first_flight_reads_cuminf[counter] <- if(!is.na(seq_total_output$first_flight_reads[counter])) sum(output2$n_SE_Output[1:seq_total_output$first_flight_reads[counter]])/population_size else 0 
    seq_total_output$avg_flight_reads_cuminf[counter] <- if(!is.na(seq_total_output$avg_flight_reads[counter])) sum(output2$n_SE_Output[1:seq_total_output$avg_flight_reads[counter]])/population_size else 0 
    seq_total_output$agg_flight_reads_cuminf[counter] <- if(!is.na(seq_total_output$agg_flight_reads[counter])) sum(output2$n_SE_Output[1:seq_total_output$agg_flight_reads[counter]])/population_size else 0 
    seq_total_output$first_flight_inf_cuminf[counter] <- if(!is.na(seq_total_output$first_flight_inf[counter])) sum(output2$n_SE_Output[1:seq_total_output$first_flight_inf[counter]])/population_size else 0 
    seq_total_output$avg_flight_inf_cuminf[counter] <- if(!is.na(seq_total_output$avg_flight_inf[counter])) sum(output2$n_SE_Output[1:seq_total_output$avg_flight_inf[counter]])/population_size else 0 
    
    counter <- counter + 1  
  }
  print(i)
}
saveRDS(seq_total_output, "outputs/seqTotal_sensitivity_analysis.rds")

## Summarising the Output
seq_tot_df_indiv_sim <- seq_total_output %>%
  pivot_longer(-c(seq_total, num_reads, stochastic_realisation), values_to = "am_time_to_detection", names_to = "metric")
seq_tot_df_summary <- seq_tot_df_indiv_sim %>%
  group_by(seq_total, metric) %>%
...
ggplot() +
  geom_line(data = seq_tot_df_summary, aes(x = seq_total, y = avg)) +
  geom_ribbon(data = seq_tot_df_summary, aes(x = seq_total, y = avg, ymin = lower, ymax = upper), alpha = 0.2) +
  geom_line(data = beta_df_indiv_sim, aes(x = beta, y = am_time_to_detection, group = stochastic_realisation), alpha = 0.05) +
  facet_wrap(~metric, labeller = metric_labeller, scales = "free_y") +
  lims(x = c(min(seq_tot_sens), max(seq_tot_sens)), y = c(0, NA)) + 
  geom_text(data = seq_tot_df_summary, aes(x = seq_total, y = 0, label = num_not_reached, family = num_not_reached)) +
  scale_x_continuous(trans = "log10") +
  labs(x = "Parameter Value - Total Sequencing", y = "Time to Detection") +
  theme_bw()
...
seq_tot_df_indiv_sim <- seq_total_output %>%
  pivot_longer(-c(seq_total, num_reads, stochastic_realisation), values_to = "am_time_to_detection", names_to = "metric")
ggplot(seq_tot_df_indiv_sim, aes(x = seq_total, y = am_time_to_detection, group = stochastic_realisation)) +
  geom_line() +
  scale_x_continuous(trans = "log10") +
  facet_wrap(~metric)
