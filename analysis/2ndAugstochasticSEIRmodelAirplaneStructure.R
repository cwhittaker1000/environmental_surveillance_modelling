...
stoch_seir_dust <- odin::odin({
  ...
  infection_length <- (1/gamma) + (1/sigma) # thinking about overdispersion in flight taking propensity; note what the assumptions associated with this are (assuming people stay in Location B????)
  p_flight <- if ((num_flights * capacity_per_flight) > population_size) 1 else (num_flights * capacity_per_flight)/population_size
  n_inf_flight <- rbinom(I_total, dt * p_flight / infection_length) # number of infected people (E+I) taking flight
...
mod <- stoch_seir_dust$new(beta = 2, gamma = 1, sigma = 1, population_size = 10^6, start_infections = 5,
                           capacity_per_flight = 100, num_flights = 200, num_flightsAB = 25, 
...
lines(output[, "time"], output[, "I_total"], ylim = c(0, 10^6), col = "blue")
